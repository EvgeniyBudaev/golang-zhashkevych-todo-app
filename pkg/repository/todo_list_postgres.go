...
func (r *TodoListPostgres) Create(userId int, list models.TodoList) (int, error) {
	tx, err := r.db.Begin() // tx - создание транзакции через метод Begin()
	if err != nil {
		return 0, err
	}
	var id int
	...
	if err := row.Scan(&id); err != nil {
		tx.Rollback() // Rollback - Откатывает все изменения БД до начала выплнения транзакции
		return 0, err
	}
	...
	return id, tx.Commit() // Commit - применяет изменения к БД и заканчивает транзакцию
}
